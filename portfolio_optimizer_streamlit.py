...
st.sidebar.markdown("""
**📌 Como usar:**
1. Configure os ativos na aba **Ativos**
2. Clique em **Buscar Dados**
3. Veja parâmetros calculados
4. Otimize o portfólio
""")
...
# Main Content
# =====================
tab_assets, tab_data, tab_params, tab_opt, tab_frontier = st.tabs([
    "⚙️ Ativos", "📊 Dados", "📈 Parâmetros", "🎯 Otimização", "📉 Fronteira Eficiente"
])

# =====================
# Tab: Assets Configuration
# =====================
with tab_assets:
    st.subheader("Configuração de Ativos")
    
    st.markdown("""
    Configure os ativos que serão utilizados na otimização do portfólio.
    
    | Coluna | Descrição |
    |--------|-----------|
    | **Classe** | Nome da classe de ativo |
    | **Fonte** | `yfinance` para ETFs/ações ou `bcb` para séries do Banco Central |
    | **Identificador** | Ticker (ex: BOVA11.SA) ou código SGS do BCB (ex: 4389) |
    | **Retorno Esperado** | Retorno anual esperado (%) - usado na otimização |
    | **Min/Max** | Limites de alocação (%) para cada ativo |
    """)
    
    st.markdown("### 📊 Mapeamento de Ativos")
    
    # Asset mapping editor (full width)
    st.session_state["asset_mapping"] = st.data_editor(
        ...
        column_config={
            ...
            "Fonte": st.column_config.SelectboxColumn("Fonte", options=["yfinance", "bcb"], width="small"),
            "Identificador": st.column_config.TextColumn("Ticker/Código", width="medium"),
            "Retorno Esperado (%)": st.column_config.NumberColumn("μ Esperado (%)", min_value=-50, max_value=100, step=0.5, format="%.2f"),
    ...
    with col1:
        fetch_data = st.button("🔄 Buscar Dados de Mercado", type="primary", use_container_width=True)
    ...
    with col3:
        ...
        csv_mapping = st.session_state["asset_mapping"].to_csv(index=False).encode()
        st.download_button(
            "📥 Exportar CSV",
            data=csv_mapping,
            file_name="mapeamento_ativos.csv",
            mime="text/csv",
            use_container_width=True
        )
    
    # Reference info
    with st.expander("📚 Referência de Tickers e Códigos BCB"):
        col1, col2 = st.columns(2)
        
        with col1:
            st.markdown("""
            **ETFs Brasileiros (yfinance):**
            | Ticker | Descrição |
            |--------|-----------|
            | BOVA11.SA | Ibovespa |
            | IVVB11.SA | S&P 500 (BRL) |
            | IMAB11.SA | IMA-B (Inflação) |
            | IRFM11.SA | IRF-M (Pré-fixado) |
            | CPTI11.SA | Crédito Privado |
            | DIVO11.SA | Dividendos |
            | SMAL11.SA | Small Caps |
            | HASH11.SA | Criptomoedas |
            """)
    ...
    # Show status if data was fetched
    if fetch_data:
        st.session_state["trigger_fetch"] = True
    
    if "prices" in st.session_state:
        st.success(f"✅ Dados carregados: {len(st.session_state['prices'])} dias, {len(st.session_state['prices'].columns)} ativos")
...
with tab_data:
    ...
    # Check if fetch was triggered from Assets tab
    should_fetch = st.session_state.get("trigger_fetch", False)
    
    if should_fetch:
        st.session_state["trigger_fetch"] = False  # Reset trigger
        with st.spinner("Buscando dados de mercado..."):
            ...
            if not prices.empty:
                ...
                st.error("❌ Não foi possível carregar os dados. Verifique o mapeamento de ativos.")
    ...
    if "prices" in st.session_state:
        ...
        st.info("👆 Configure os ativos na aba **Ativos** e clique em **Buscar Dados de Mercado** para começar.")
